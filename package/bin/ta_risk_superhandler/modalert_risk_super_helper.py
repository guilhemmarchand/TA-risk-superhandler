...
def process_event(helper, *args, **kwargs):
                        ...
                        if risk_object_value:
                            ...
                            if not format_separator and len(risk_object_mv_field) == 0 and type(record[risk_object]) != list:
                                ...
                                # stored in a native list
                                    ...
                                    if format_separator:
                                        helper.log_warning("uc_ref=\"{}\", the risk_object=\"{}\" has a format_separator=\"{}\" but it comes a multivalue field instead, this configuration is likely incorrect".format(record[uc_ref_field], risk_object, format_separator))
    ...
    if not all_new_records:
        helper.log_debug("Not triggering any action, all risk objects failed to be extracted, please verify the event and the risk definition.")
        run_riskcollect = False
